...
- `entry_switch` / `exit_switch`: alert の何桁目を入庫/出庫として見るか（1〜4、`config` 指定）。
- `active_value`: alert 文字で ACTIVE を意味する値（`"0"` または `"1"`）。LinkBase が値を反転する仕様のため既定は `"0"`。
...
- LinkBase の送信形式・送信契機（=状態変化時のみ）はソース確認済み。`active_value` の絶対極性のみ実機観測で確定。
